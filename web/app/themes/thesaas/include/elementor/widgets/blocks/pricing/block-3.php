...
class The_Pricing_Block_3 {
  ...
  public function controls( $widget ) {
    ...
    $widget->panel( 'pricing_plans', [ 'plans' => $default_plans ] );
  }
  ...
  public function html( $widget ) {
    ...
    $radio_id = rand(1, 999);
    $plans = $settings['t3_pricing_plan'];
    ...
    <?php $widget->html('section_tag'); ?>
      <?php $widget->html('section_header'); ?>
        ...
        <?php if ( 'yes' == $settings['t3_yearly_pricing_plan'] ) : ?>
        <div class="text-center mb-70">
          <div class="btn-group" data-toggle="buttons">
            <label class="btn btn-round btn-outline <?php echo esc_attr( $settings['t3_plans_toggle_color'] ); ?> active" style="min-width: 150px">
              <input type="radio" name="pricing-<?php echo $radio_id; ?>" value="monthly" autocomplete="off" checked> <?php echo $settings['t3_monthly_text'] ?>
            </label>
            <label class="btn btn-round btn-outline <?php echo esc_attr( $settings['t3_plans_toggle_color'] ); ?>" style="min-width: 150px">
              <input type="radio" name="pricing-<?php echo $radio_id; ?>" value="yearly" autocomplete="off"> <?php echo $settings['t3_yearly_text'] ?>
            </label>
          </div>
        </div>
        <?php endif; ?>
            ...
            <div class="<?php echo $col_class; ?>">
              <div class="pricing-3">
                <?php if ( 'yes' == $plan['popular'] ) : ?>
                  <p class="plan-name text-info"><?php echo $plan['name']; ?></p>
                <?php else : ?>
  ...
  public function javascript( $widget ) {
    ...
    <?php $widget->js('section_tag'); ?>
      ...
      <# if ( 'yes' == settings.t3_yearly_pricing_plan ) { #>
      <div class="text-center mb-70">
        <div class="btn-group" data-toggle="buttons">
          <label class="btn btn-round btn-outline {{ settings.t3_plans_toggle_color }} active" style="min-width: 150px">
            <input type="radio" name="pricing-<?php echo $radio_id; ?>" value="monthly" autocomplete="off" checked> {{{ settings.t3_monthly_text }}}
          </label>
          <label class="btn btn-round btn-outline {{ settings.t3_plans_toggle_color }}" style="min-width: 150px">
            <input type="radio" name="pricing-<?php echo $radio_id; ?>" value="yearly" autocomplete="off"> {{{ settings.t3_yearly_text }}}
          </label>
        </div>
      </div>
      <# } #>
        ...
        var plans = settings.t3_pricing_plan;
        ...
        if ( plans ) {
          ...
          #>
            <div class="{{ col_class }}">
              <div class="pricing-3">
                <# if ( 'yes' == plan.popular ) { #>
                  <p class="plan-name text-info">{{{ plan.name }}}</p>
                <# } else { #>
