						...
						<div id="post-<?php the_ID(); ?>" <?php post_class('card card-hover-shadow'); ?>>
							...
							<div class="card-block">
								...
								<?php
									if ( has_excerpt() ) {
										the_excerpt();
										echo '<p class="mb-0"><a class="more-link" href="'. get_permalink( get_the_ID() ) .'"><span>'. esc_html__( 'Apply now', 'thesaas' ) .' <i class="fa fa-chevron-right"></i></span></a></p>';
									}
									else {
										the_content( sprintf(
											'<span>'. esc_html__( 'Apply now', 'thesaas' ) .' <i class="fa fa-chevron-right"></i></span>'
										) );
